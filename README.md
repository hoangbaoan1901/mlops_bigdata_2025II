...
1. **Port Forward KServe Service** (if needed)
   ```bash
   kubectl port-forward svc/bankchurn-model-predictor-default -n kubeflow-user-example-com 8080:80
   ```

2. **Test Model Prediction**
   ```bash
   curl -X POST http://localhost:8080/v2/models/bankchurn-model/infer \
...
3. **Expected Response**
   ```json
   {
     ...
     "id": "3c17331d-dd7c-432e-9c91-79d038af44fd",
     "parameters": {
       "content_type": "np"
     },
     "outputs": [
